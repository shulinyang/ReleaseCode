...
for user_index=[1]
    ...
    for gesture_index = 3:3
        for position_index = 1:1
            for orientation_index = 1:1
                for instance_index = instance_list
                    ...
                    try
                        ...
                        for kk = [6]
                            ...
                            for mm = pca_list
                                ...
                                %                                         plot_data=squeeze(plot_line_h(user_index,instance_fig_index,line_index_h,:,:));
                                %                                         hold on;
                                ...
                                %                                         plot_data=squeeze(plot_line_v(user_index,instance_fig_index,line_index_v,:,:));
                                %                                         hold on;
                                ...
                                %                                         plot_line_h(user_index,instance_fig_index,line_index_h,1,:)=sequence(1:size(plot_line_h,5));
                                %                                         plot_line_h(user_index,instance_fig_index,line_index_h,2,:)=interp1(data(1,:),data(2,:),  plot_line_h(user_index,instance_fig_index,line_index_h,1,:));
                                %
                                %                                         hold on;
                                %                                         plot_data=squeeze(plot_line_h(user_index,instance_fig_index,line_index_h,:,:));
                                %                                         h(line_index_h)=plot3(plot_data(1,:),plot_data(2,:),plot_data(3,:)+0.1,':','color',color_dot(line_index_h,:),'linewidth',5);
                                ...
                                %                                         plot_line_v(user_index,instance_fig_index,line_index_v,2,:)=sequence(1:size(plot_line_v,5));
                                %                                         plot_line_v(user_index,instance_fig_index,line_index_v,1,:)=interp1(data(1,:),data(2,:),  plot_line_v(user_index,instance_fig_index,line_index_v,1,:));
                                %                                         hold on;
                                %                                         plot_data=squeeze(plot_line_v(user_index,instance_fig_index,line_index_v,:,:));
                                %                                         h(4)=plot3(plot_data(1,:),plot_data(2,:),plot_data(3,:)+0.1,':','color',color_dot(4,:),'linewidth',5);
                                %                                         datacursormode off
                                ...
                                %                                 set(gcf,'WindowStyle','normal','Position', [250*(pca_fig_index-1),300*(instance_fig_index-1),300,250]); % window size
                                ...
                                set(gca,'xTick',0:300:size(doppler_spectrum,4));
                                xlabel('Time (ms)','FontWeight','bold','FontSize',40); % x label
                                ylabel('Frequency Shift(Hz)','FontWeight','bold','FontSize',40); % y label
                                
                                saveas(gcf,[figures_dir,save_type,'-',data_file_name,'-r', num2str(kk),'-p', num2str(mm), '.fig']);
                                saveas(gcf,[figures_dir,save_type,'-',data_file_name,'-r', num2str(kk),'-p', num2str(mm), '.png']);
